...
counter++;
		...
		leds[0] = 0xFF ;//counter;//row 1, from center
		leds[1] = 0xFF ;//counter;//row 2, from side
		leds[2] = 0xFF ;//counter;//row 3, from center
		leds[3] = 0xFF ;//counter;//row 4, from center
		leds[4] = 0xFF ;//counter++;//row 5, from center
 	...
 	if ( bitCounter > 5 ) 
	{
	 ...
	 leds[0] = ~( 0B00000001 << (bitCounter) ) ;//13 o'clock row, from outside
//problem 2th from center, and 2 outside

	leds[1] = ~( 0B00000001 << (bitCounter) );//- counter;//16 o'clock row, from center
//seems ok

	leds[2] = ~( 0B00000001 << (bitCounter) ) ;//- counter;//20 o'clock row, from center
//seems ok, contamination 4 and 3

	 leds[3] = ~( 0B00000001 << (bitCounter) ) ;// 15 o'clock row, from outside
//wire problem 2 from center last two outside, contamination on led[4]

	leds[4] = ~( 0B00000001 << (bitCounter++) );//18 o'clock row, from center down pointing -1,-2, 0B11101100 = 5th, 
  	//wire problem led 3 and led 4 from center
